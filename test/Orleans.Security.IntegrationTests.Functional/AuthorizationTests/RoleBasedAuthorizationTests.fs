...
let ``Multiple roles as a comma separated list should work when the user has both roles`` (userName: string)
...
let ``Multiple roles as a comma separated list shouldn't work when the user has only one role`` (userName: string)
    (password: string) (scope: string) =
    async {
        // Arrange
        let! accessTokenResponse = IdentityServer4Client.getAccessTokenWAsync userName password scope |> Async.AwaitTask
...
[<Theory>]
[<InlineData("Carol", "Pass123$", "Api1 Orleans")>]
let ``Multiple roles applied as multiple attributes should work when the user has both roles`` (userName: string)
    (password: string) (scope: string) =
    async {
        // Arrange
        let! accessTokenResponse = IdentityServer4Client.getAccessTokenWAsync userName password scope |> Async.AwaitTask
...
[<Theory>]
[<InlineData("Alice", "Pass123$", "Api1 Orleans")>]
let ``Multiple roles applied as multiple attributes shouldn't work when the user has only one role`` (userName: string)
    (password: string) (scope: string) =
    async {
        // Arrange
        let! accessTokenResponse = IdentityServer4Client.getAccessTokenWAsync userName password scope |> Async.AwaitTask
